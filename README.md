...
A token is used to authenticate a user, granting them permission to query the API. 

- [ ] What steps can you take in your web apps to keep your data secure?

Tokens should not be stored in local storage. Private routes can be created to protect parts of the app. 

- [ ] Describe how web servers work.

Web servers listen for client requests to access their data. If applicable and once authenticated, the server returns content to 
the client.

- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

C - post request after creating username and password.
R - get request then consuming the api.
U - put request to edit data.
D - delete request to delete data.
